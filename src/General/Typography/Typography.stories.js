...
export const Headings = () => {
  const dom = htags.map(htag => `<${htag}>${htag.toUpperCase()} title</${htag}>`).join('\n');
  return dom;
};

export const LeadParagraph = () => {
  const dom = `<p class="lead">This is a lead paragraph</p>\n<p>And this is a regular paragraph</p>`;
  return dom;
};
...
export const InlineElements = () => {
  return `<p>You can use the mark tag to <mark>highlight</mark> text.</p>
